...
!
    ...
    subroutine TRANSFER_CALC
        ...
        REAL(kind=dp), DIMENSION(:,:), ALLOCATABLE :: transfer_vel_arr    ! Rotated states of the target
        ...
        allocate(transfer_vel_arr(num_lines, 6))
        ...
        read_loop: do itercount = 1, num_lines
            ...
            ! if (omp_get_thread_num() .eq. 0 .and. num_iters .eq. 0) then
                ...
                ! write(*,*) "Performing epoch", t, "with time", tt, "on processor count", omp_get_num_threads()

            ! end if

            ! Get this target state

            state_targ = state_targ_arr(itercount, :)
            state_rot  = state_rot_arr(itercount, :)
        ...
        open(unit=69, file="transferVelocities.csv")
