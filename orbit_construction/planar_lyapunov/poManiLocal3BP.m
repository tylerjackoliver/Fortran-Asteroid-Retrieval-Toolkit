...
% Add an initial guess for the stm to the state vector

x0po = [x0po reshape(eye(6), [], 36)];

% get monodromy matrix 
[tt,xx] = ode45('stm_ode', [0 T], x0po, OPTIONS);
phi_T = reshape(xx(end, N+1:end), N, N);
PHI = xx(:,N+1:end);
...
phi_frac=reshape(PHI(mfrac,:),N,N);
...
%  mfrac
  ...
  po_frac = xx(mfrac,1:N)'; % point on p.o.
  x0W_all(mfrac,1:N) = po_frac(:)' + d*MAN(:)'; % displace onto manifold

  
%   [x,t] = trajGet3BP(x0W_all(mfrac,1:N)',tb,tf,mu,OPTIONS);
% Negative time!
  [t, x] = ode45('threeBodyEOM', [tf, tb], x0W_all(mfrac, 1:N)', OPTIONS);
