...
function [dset, Y] = manifold(y0,t0, mu)
    %% Initialisation

    N = 360;                                                                    % Number of discretisations to compute
    ...
    [XS_in, XS_out, Y] = manifold_mono(mu, y0, t0, N, xpert);             % XS_in, XS_out, XU_in, XU_out, Monodromy matrix
    ...
    for ii = 1:N/12:N
        ...
        [~, Y1] = ode45('threeBodyEOM', [0 -6*pi], XS_in(:,ii), ...
                 OPTIONS_events);
             
        [~, Y2] = ode45('threeBodyEOM', [0 -6*pi], XS_out(:,ii), ...
    ...
    elseif (test_vals_2(:) > 0)
        ...
        fprintf("Neither directions for eigvenvalue perturbations are OK!\n");
%         pause;
%         error("Neither directions for eigenvalue perturbations are OK!");
    ...
    % Now integrate call of the 'correct' directions
    
    %
    % As of 12/3, integration is now performed using a Fortran routine.
    % Below is therefore deleted.
    %
