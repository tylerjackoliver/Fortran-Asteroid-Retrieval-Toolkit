    ...
    contains
        ...
        subroutine bspline_interpolate(t_end, n_mnfd, J, interpolated_state_vector)
            ...
            do dimension = 2, 7 ! Loop through state vector
               ...
               call MPI_COMM_RANK(MPI_COMM_WORLD, mpi_id_world, mpi_err)
                ...
                call db2val(t_end, n_mnfd, idx, idy, tx, ty, nx, ny, kx, ky, bcoef, interpolated_state_vector(dimension-1), iflag, &
        ...
        subroutine bspline_interpolate_time(t_end, n_mnfd, J, tmani)
            ...
            do dimension = 1,1 ! Loop through state vector
                ...
                call db2val(t_end, n_mnfd, idx, idy, tx, ty, nx, ny, kx, ky, bcoef, tmani, iflag, &
                            inbvx, inbvy, iloy, w1, w0, extrap) 
        ...
        subroutine b_spline_interpolate_perturbed_conds(t_end, n_mnfd, J, interpolated_state_vector, tmani)

            implicit none

            real(kind=dp), intent(in)           :: n_mnfd                           ! Along-orbit direction interpolant
            real(kind=dp), intent(in)           :: t_end                            ! Back-direction interpolant

            integer, intent(in)                 :: J                                ! Which orbit we're studying (bruting over J)

            real(kind=dp), intent(out)          :: tmani

            real(kind=dp)                       :: temp_state_vector(6)             ! Holds time as well
            real(kind=dp), intent(out)          :: interpolated_state_vector(6)     ! Final interpolated structure 
            ...
            do dimension = 2,7 ! Loop through state vector
                ...
                fnc_value(:) = perturbed_conds_dataset((n_mnfd_floor+bias_n_mnfd-1):(n_mnfd_ceil+bias_n_mnfd+1), &
                                            J, dimension-1) ! (tend, nmnfd)

                x_abcissae(:) = (/n_mnfd_floor+bias_n_mnfd-1, n_mnfd_floor+bias_n_mnfd, n_mnfd_ceil+bias_n_mnfd, &
                                n_mnfd_ceil+bias_n_mnfd+1/)

                call db1ink(x_abcissae, nx, fnc_value, kx, iknot, tx, bcoef, iflag)

                if (iflag .ne. 0) error STOP "Interpolation initialisation failed."

                call db1val(n_mnfd, idx, tx, nx, kx, bcoef, temp_state_vector(dimension-1), iflag, &
                            inbvx, w0, extrap) 
            ...
            call bspline_interpolate_time(t_end, n_mnfd, J, tmani)

            interpolated_state_vector = temp_state_vector
