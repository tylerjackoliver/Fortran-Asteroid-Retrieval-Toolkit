...
!
! Results in 7299652 index, 0.299m/s.
! Captures into orbit of J = 3.000603090512101.
    ...
    subroutine TRANSFER_CALC(state_can,t,tt,input_file,min_vel,f_index)
        ...
        read_loop: do
            ...
            read(99,*,iostat=io_state) state_targ(1), state_targ(2), state_targ(3), state_targ(4), state_targ(5), &
                                        state_targ(6)
    ...
    subroutine ROTATOR(state_in, epoch, tt, state_out)
        ...
        if (abs(elts(4)) < 1.0d-6) then
            ...
            elts(4) = 0
            elts(5) = elts(5) + (epoch+tt)*(2*pi)/(365.25d0*86400.d0)                                                                   ! Add on phasing equal to the transfer time                                           

        else

            elts(4) = elts(4) + (epoch+tt)*(2*pi)/(365.25d0*86400.d0)                                                                   ! Add on phasing equal to the transfer time
            elts(5) = 0
