...
!
    ...
    !   use omp_lib
        ...
        integer(kind=sp)  :: io_state                             ! File I/O status integer
        integer(kind=sp)  :: i                                    ! Loop variable
        integer(kind=sp)  :: j                                    ! Loop variable
        integer(kind=sp)  :: index                                ! Target state in input file
        integer(kind=sp)  :: max_threads
        ...
        character(len=9)  :: targ_can                             ! Target candidate string
        character(len=24) :: input_file='output_data_jacobi.dat'
        ...
        targ_can      = '3610175'
        transfer_time = 1355.d0*86400.d0
        ...
        call STR2ET('15 Nov 2020 00:00',transfer_epoch)
    ...
    subroutine TRANSFER_CALC(state_can,t,tt,input_file,min_vel,f_index)
        ...
        REAL(kind=dp)                              :: vel                 ! velocity of the target
        ...
        REAL(kind=sp)                              :: dum                 ! dummy variables
        ...
        write(*,*) 'File opened successfully. Reading...'
        ...
        read_loop: do
            ...
            read(99,*,iostat=io_state) dum, state_targ(1), state_targ(2), state_targ(3), state_targ(4), state_targ(5), &
                                        state_targ(6), dum
            ...
            if (run_ok .eqv. .false.) then
                ...
                call solve_lambert_gooding(state_can(1:3),state_targ(1:3),tt,mu,long_way,multi_rev,v1,v2,run_ok)
            ...
            if (run_ok .eqv. .false.) then
                ...
                call solve_lambert_gooding(state_can(1:3),state_targ(1:3),tt,mu,long_way,multi_rev,v1,v2,&
                    run_ok)
    ...
    subroutine ROTATOR(state_in, epoch, tt, state_out)
        ...
        if (abs(elts(4)) < 1.0d-6) then
            ...
            elts(4) = 0
            elts(5) = elts(5) + tt*(2*pi)/(365.25d0*86400.d0)                                                                   ! Add on phasing equal to the transfer time                                           

        else

            elts(4) = elts(4) + tt*(2*pi)/(365.25d0*86400.d0)                                                                   ! Add on phasing equal to the transfer time
            elts(5) = 0
