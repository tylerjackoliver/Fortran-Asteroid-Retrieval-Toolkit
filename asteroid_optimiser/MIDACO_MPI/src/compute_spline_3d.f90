module compute_spline
    ...
    contains
        ...
        subroutine bspline_interpolate(t_end, n_mnfd, J, interpolated_state_vector)
            ...
            real(kind=dp), intent(in)           :: J                                ! Which orbit we're studying (bruting over J)
            ...
            real(kind=dp)                       :: z_abcissae(4)                    ! Array of z-abcissae
            real(kind=dp)                       :: fnc_value(4, 4, 4)               ! Value of function to use in interpolating
            real(kind=dp)                       :: tx(7)                            ! Locations of the knots: size (nx + kx)
            real(kind=dp)                       :: ty(7)                            ! As above, but for the y-coordinate
            real(kind=dp)                       :: tz(7)                            ! As above, but for the z-coordinate
            real(kind=dp)                       :: bcoef(4, 4, 4)                   ! Matrix of coefficients of the b-spline interpolant
            real(kind=dp)                       :: w0(12)                           ! Work array for db2val
            real(kind=dp)                       :: w1(4)                            ! Work array for db2val
            real(kind=dp)                       :: w2(3, 3)                         ! Work array db3val
        ...
        subroutine bspline_interpolate_step(t_end, n_mnfd, J, interpolated_state_vector)
            ...
            real(kind=dp)                       :: x_abcissae(4)                    ! Array of x-abcissae
            real(kind=dp)                       :: x_abcissae_step(4)               ! Array of x-abcissae for the time-steps
            real(kind=dp)                       :: y_abcissae(4)                    ! Array of y-abcissae
            real(kind=dp)                       :: z_abcissae(4)                    ! Array of z-abcissae
            real(kind=dp)                       :: fnc_value(4, 4, 4)               ! Value of function to use in interpolating
            real(kind=dp)                       :: tx(7)                            ! Locations of the knots: size (nx + kx)
            ...
            real(kind=dp)                       :: baseline_time1                   ! First time in the dataset for that orbit (time to reach pi/8)
            real(kind=dp)                       :: baseline_time2                   ! First time in the dataset for that orbit (time to reach pi/8)
            real(kind=dp)                       :: baseline_time3                   ! First time in the dataset for that orbit (time to reach pi/8)
            real(kind=dp)                       :: baseline_time4                   ! First time in the dataset for that orbit (time to reach pi/8)
            real(kind=dp)                       :: actual_time1                     ! Actual time in the dataset for that orbit
            real(kind=dp)                       :: actual_time2                     ! Actual time in the dataset for that orbit
            real(kind=dp)                       :: actual_time3                     ! Actual time in the dataset for that orbit
            real(kind=dp)                       :: actual_time4                     ! Actual time in the dataset for that orbit
            real(kind=dp)                       :: orbit_number(4)
            real(kind=dp)                       :: nominal_time_step1               ! nominal time step for timestep <-> absolute conversions
            real(kind=dp)                       :: nominal_time_step2               ! nominal time step for timestep <-> absolute conversions
            real(kind=dp)                       :: nominal_time_step3               ! nominal time step for timestep <-> absolute conversions
            real(kind=dp)                       :: nominal_time_step4               ! nominal time step for timestep <-> absolute conversions
            real(kind=dp)                       :: t_end_idx1                       ! Index relating to the time-step in the database
            real(kind=dp)                       :: t_end_idx2                       ! Index relating to the time-step in the database
            real(kind=dp)                       :: t_end_idx3                       ! Index relating to the time-step in the database
            real(kind=dp)                       :: t_end_idx4                       ! Index relating to the time-step in the database
            real(kind=dp)                       :: t_end_abs
            ...
            baseline_time1 = dataset(1, n_mnfd_floor+bias_n_mnfd, J_floor+bias_J-1, 1)
            baseline_time2 = dataset(1, n_mnfd_floor+bias_n_mnfd, J_floor+bias_J, 1)
            baseline_time3 = dataset(1, n_mnfd_floor+bias_n_mnfd, J_ceil+bias_J, 1)
            baseline_time4 = dataset(1, n_mnfd_floor+bias_n_mnfd, J_ceil+bias_J+1, 1)
            ...
            actual_time1 = dataset(x_abcissae_step(1), n_mnfd_floor+bias_n_mnfd, J_floor+bias_J-1, 1)
            actual_time2 = dataset(x_abcissae_step(2), n_mnfd_floor+bias_n_mnfd, J_floor+bias_J, 1)
            actual_time3 = dataset(x_abcissae_step(3), n_mnfd_floor+bias_n_mnfd, J_ceil+bias_J, 1)
            actual_time4 = dataset(x_abcissae_step(4), n_mnfd_floor+bias_n_mnfd, J_ceil+bias_J+1, 1)
...
30          if (x_abcissae(1) .lt. t_end) then
        ...
        subroutine bspline_interpolate_time(t_end, n_mnfd, J, tmani)
            ...
            real(kind=dp), intent(out)          :: tmani                            ! Final interpolated structure 
