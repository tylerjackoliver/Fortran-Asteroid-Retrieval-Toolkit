...
!     INITIALISE BOUNDS
    ...
    MIN(1)=TRANSFER_EPOCH*.9998D0
    MIN(2)=1239.d0*86400.d0*0.98D0
    MAX(1)=TRANSFER_EPOCH*1.0002D0
    MAX(2)=1239.d0*86400.d0*1.02D0
...
END
    ...
    SUBROUTINE FUNCT(X, MIN_VEL, NPARM, M)
        ...
        ! Compute the candidate position - MODIFIED to always return the ephemeris time

        call CANDIDATE_POSITION_MOD(targ_can, transfer_epoch, tt, state_can)
