...
!
    ...
    !   use omp_lib
        ...
        transfer_time = 1241.d0 * 86400.d0
    ...
    subroutine TRANSFER_CALC(state_can,t,tt,input_file,min_vel,f_index)
        ...
        REAL(kind=dp), DIMENSION(:,:)              :: v1                  ! ALLOCATABLE attribute required by the Fortran Astrodynamics Toolkit
        REAL(kind=dp), DIMENSION(:,:)              :: v2                  ! ALLOCATABLE attribute required by the Fortran Astrodynamics Toolkit
        ...
        read_loop: do itercount = 1, num_lines
            ...
            call LAMBERTBATTIN(state_can(1:3), state_rot(1:3), 'S', 
            ...
            do j = 1, num_rows
                ...
                if (transfer_vel < min_vel) then
                    ...
                    write(*,*) "Found new minimum:", min_vel
